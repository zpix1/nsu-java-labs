...
public class SettingsManager {
    private final Map<String, GameData.Settings> settingsTypes;

    // TODO: Tried to use reflexion api here, but it's just evil

    public SettingsManager(String filename) {
        Properties props = new Properties();
        settingsTypes = new HashMap<>();
        try {
            props.load(this.getClass().getResourceAsStream(filename));
        } catch (IOException e) {
            e.printStackTrace();
        }
        for (var propName: Collections.list(props.keys())) {
            var splitted = propName.toString().split("\\.");
            var type = splitted[0];
